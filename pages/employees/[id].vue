  ...
  <div>
    <UBreadcrumb 
      :links="[
        ...
        { label: employee ? `${employee.first_name} ${employee.last_name}` : 'Employee Details' }
      ]" 
      class="mb-6"
    />
    ...
    <!-- Employee not found -->
    <UCard v-else-if="!employee" class="p-12 flex flex-col items-center justify-center">
      <UIcon name="i-heroicons-exclamation-triangle" class="w-12 h-12 text-amber-400 mb-4" />
      <h3 class="text-lg font-medium mb-2">Employee Not Found</h3>
      <p class="text-gray-500 mb-6 text-center">The employee you're looking for doesn't exist or you don't have permission to view it.</p>
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1">
        <UCard>
          <div class="flex flex-col items-center text-center mb-4">
            ...
            <p class="text-gray-500">{{ employee.job_title || 'No job title' }}</p>
          ...
          <div class="border-t pt-4 mt-4">
            <div class="space-y-3">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium">{{ managerName || 'Not assigned' }}</span>
              </div>
              <div class="flex justify-between">
          ...
          <div class="mt-6 border-t pt-4">
            ...
            <p class="text-gray-600 text-sm">{{ employee.bio || 'No bio available' }}</p>
          </div>

          <template #footer>
            <div class="flex justify-between">
              <UButton color="gray" variant="ghost" icon="i-heroicons-pencil-square" @click="editEmployee">
                Edit Profile
              </UButton>
              <UButton color="blue" icon="i-heroicons-clipboard-document-check" @click="startEvaluation">
                Evaluate
              </UButton>
            </div>
          </template>
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <UCard>
          ...
          <div v-if="recentEvaluations.length === 0" class="py-6 text-center text-gray-500">
            No evaluations found for this employee
          </div>
          
          <ul v-else class="divide-y divide-gray-200 dark:divide-gray-700">
            <li v-for="evaluation in recentEvaluations" :key="evaluation.id" class="py-4">
              <div class="flex justify-between">
                <div>
                  <div class="font-medium">{{ formatDate(evaluation.evaluation_date) }}</div>
      ...
      <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark" class="-my-1" aria-label="Close" @click="isEditModalOpen = false" />
          </div>
        </template>
        ...
        <UForm :schema="schema" :state="form" @submit="submitForm">
          ...
          <div class="flex justify-end space-x-2 mt-6">
            <UButton type="button" color="gray" variant="ghost" @click="isEditModalOpen = false">
              Cancel
            </UButton>
...
onMounted(async () => {
  ...
  if (employee.value) {
    ...
    if (employee.value.manager_id) {
      const manager = managers.value.find(m => m.id === employee.value.manager_id);
      if (manager) {
...
async function fetchEmployee() {
  try {
    ...
    employee.value = await $client.v1.employees.get.query({ id: employeeId.value });
  } catch (error: any) {
...
function editEmployee() {
  ...
  form.hireDate = employee.value.hire_date ? formatDateForInput(employee.value.hire_date) : '';
